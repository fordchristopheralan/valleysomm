...
import { ChevronRight, Check, X, Loader2, Calendar } from 'lucide-react';
...
const QUESTIONS: Question[] = [
  ...
  {
    id: 'occasion',
    question: 'Any special occasion? (optional)',
    type: 'text',
    placeholder: 'e.g., birthday, anniversary, bachelorette...',
    optional: true
  },
  {
    id: 'stops',
...
export default function Questionnaire({ onCancel }: QuestionnaireProps) {
  ...
  const handleDateRange = () => {
    const newAnswers = {
      ...answers,
      visitDateStart: startDate || undefined,
      visitDateEnd: endDate || undefined,
    };
    setAnswers(newAnswers);
      ...
      >
            ...
            {startDate && endDate && startDate !== endDate && (
              <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
                <p className="text-sm text-purple-900 font-medium">
                  🎉 Multi-day trip detected! We'll organize your {Math.ceil((new Date(endDate).getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1}-day journey with themed days and accommodation recommendations.
                </p>
              </div>
            )}
          </div>
        ...
        {currentQ.type === 'text' && (
          <div>
            ...
            />
            {currentQ.optional && (
              <p className="text-sm text-gray-500 mt-2">Optional - helps us personalize your experience</p>
            )}
          </div>
        )}
        ...
        {/* Continue button for multi-select, date-range, and text */}
        {(currentQ.type === 'multiple' || currentQ.type === 'date-range' || currentQ.type === 'text') && (
          ...
          >
            {currentQ.optional && !startDate && !textValue ? 'Skip' : isLastStep ? 'Build My Trail' : 'Continue'} 
