...
export default function TrailPage() {
  ...
  useEffect(() => {
    ...
    console.log('🔍 Loading trail:', trailId); // DEBUG

    const fetchTrail = async () => {
      try {
        console.log('🔍 Fetching from:', `/api/trails/${trailId}`); // DEBUG
        ...
        if (response.ok) {
          ...
          console.log('🔍 Trail data:', data); // DEBUG
          setTrail(data);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">Trail ID: {params?.id}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-gray-500 mb-6">Trail ID: {params?.id}</p>
          <button
            onClick={handleReset}
